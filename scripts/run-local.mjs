...
// production: the TTL is auto-extended by the Hardhat block.timestamp drift so
// orders aren't born expired, and the price-move requote gate stays off because
...
// static seeded rate, so that gate never fires and the only thing rotating
// nonces is the slower age gate (~ttl - repost_lead). A slice consumed by a
// fill then lingers in the book for up to that age, and re-filling it reverts
// with InvalidNonce. Default to 0 locally so every tick re-signs with a fresh
// nonce — which is what a price-gated bot effectively does in production, where
// the live feed moves and rotates nonces on its own. Set this (or
// REFRESH_THRESHOLD_BPS) only if you've wired up a moving local feed.
const STITCH_LOCAL_REFRESH_THRESHOLD_BPS = nonNegativeIntEnv(
  'STITCH_LOCAL_REFRESH_THRESHOLD_BPS',
  nonNegativeIntEnv('REFRESH_THRESHOLD_BPS', 0)
)
...
// The bot signs Permit2 deadlines off its wall clock (unix_now + ttl_secs), but
// the local Hardhat chain checks them against block.timestamp — which only moves
// forward and runs minutes ahead of wall time once tests or scripts bump EVM
// time. A short production-like TTL would then mint already-expired orders that
// revert on the first fill with SignatureExpired. Measure the skew once and add
// it to the TTL so the written deadline still grants STITCH_LOCAL_TTL_SECS of
// fillable life from chain time. Production chains track wall time (drift ≈ 0),
// so the effective TTL there stays the bot's short value.
const wallNow = Math.floor(Date.now() / 1000)
const { timestamp: chainTs } = await pub.getBlock()
const CHAIN_DRIFT_SECS = Math.max(0, Number(chainTs) - wallNow)
const EFFECTIVE_TTL_SECS = STITCH_LOCAL_TTL_SECS + CHAIN_DRIFT_SECS
...
const ttlNote =
  CHAIN_DRIFT_SECS > 0
    ? `${EFFECTIVE_TTL_SECS}s (${STITCH_LOCAL_TTL_SECS}s + ${CHAIN_DRIFT_SECS}s chain drift)`
    : `${EFFECTIVE_TTL_SECS}s`
console.log(
  `  production-like defaults: ttl=${ttlNote}, ` +
