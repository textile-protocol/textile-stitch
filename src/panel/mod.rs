...
//! Stitch bots on one host from a browser.
//!
//! Bots are supervised either through the Docker Engine API (server installs)
//! or as local child processes (desktop tray app).
//!
...
//!
//! Container lifecycle goes through the [`docker::DockerApi`] trait rather than
//! shelling out to `docker compose`, so the panel can adopt an existing
//! hand-written compose fleet without restarting it, and can't mangle the
//! operator's compose file. A generated (never round-tripped) compose export
//! keeps that file a valid recovery artifact for Docker installs.
